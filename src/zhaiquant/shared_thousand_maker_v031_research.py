...
from .maker_paper import SHARED_THOUSAND_POLICY_V031_CANDIDATE
...
MODEL_ID = SHARED_THOUSAND_POLICY_V031_CANDIDATE.model_id
PARENT_MODEL_ID = SHARED_THOUSAND_POLICY_V031_CANDIDATE.parent_model_id


def replay_shared_thousand_v031_day(
    ...
    """Replay v0.3 allocation with the repaired v0.11 exit-gap fallback."""
...
def run_shared_thousand_v031_matrix(
    config: AppConfig, *, dates: tuple[str, ...],
    bond_codes: tuple[str, str] = DEFAULT_BOND_CODES,
    parameters: AllocationParametersV03 | None = None,
) -> dict[str, Any]:
    """Run the causal daily matrix without changing v0.3's allocator."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Read-only causal replay for shared 1,000-bond maker v0.31."
        ),
    )
    ...
    result = run_shared_thousand_v031_matrix(
