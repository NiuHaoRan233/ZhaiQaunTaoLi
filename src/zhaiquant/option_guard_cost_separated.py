"""Offline v3: preserve v2 fee300 admission boundaries while accounting actual fees.
...
FAMILY = 'probe_option_dadao_guard_20260909_v3'
...
def entry_reasons(e, f, profile, quality_buffer_cents, tick_cents, queued):
    if profile == 'control': return []
    reasons = []
    spread = e.ask - e.bid
    inward = 0 if queued or spread <= tick_cents else 2 * tick_cents
    edge = spread - inward - quality_buffer_cents
...
def run(events, *, code, mode, profile, fee_cents=170, initial_cents=1_000_000,
        tick_cents=100, feature_rows=None, capacity=1, quality_buffer_cents=600):
    assert profile in PROFILES and mode in MODES and capacity > 0 and fee_cents >= 0
    assert isinstance(quality_buffer_cents, int) and quality_buffer_cents >= 0
    delay = 1000 if mode.endswith('d1000') else 500 if mode.endswith('d500') else 0
    queued = mode.startswith('queue')
    model_id = f'{FAMILY}_{profile}_{mode}_q{capacity}_b{quality_buffer_cents}_f{fee_cents}'
    ...
    for e, f in zip(events, feature_rows):
        ...
        if valid:
            reasons = entry_reasons(e, f, profile, quality_buffer_cents, tick_cents, queued) if not inventory else []
    ...
    s = dict(model_id=model_id, code=code, mode=mode, capacity=capacity, fee_per_side_cny=fee_cents/100,
        ...
        profile=profile, quality_buffer_cents=quality_buffer_cents, blocked_frames=dict(blocked), cancel_requests=sum(o['side']=='cancel' for o in orders),
