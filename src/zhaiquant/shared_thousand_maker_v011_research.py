...
from .maker_paper import SHARED_THOUSAND_POLICY_V011_CANDIDATE
...
MODEL_ID = SHARED_THOUSAND_POLICY_V011_CANDIDATE.model_id
PARENT_MODEL_ID = SHARED_THOUSAND_POLICY_V011_CANDIDATE.parent_model_id
DEFAULT_SWITCH_MINIMUM_CNY = 100.0
DEFAULT_TIE_TOLERANCE_CNY = 10.0


def replay_shared_thousand_v011_day(
    ...
    """Replay v0.11 with v0.1 allocation and repaired live exits."""
    ...
    return replay_one_hand_day(
        ...
        priority_policy=SHARED_THOUSAND_POLICY_V011_CANDIDATE,
        shared_capacity_bonds=SHARED_THOUSAND_BONDS,
    )


def run_shared_thousand_v011_matrix(
    config: AppConfig, *, dates: tuple[str, ...],
    bond_codes: tuple[str, str] = DEFAULT_BOND_CODES,
    parameters: AllocationParameters | None = None,
) -> dict:
    """Compare v0.11 without inheriting v0.2/v0.3 allocation changes."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Read-only causal replay for shared 1,000-bond maker v0.11."
        ),
    )
    ...
    result = run_shared_thousand_v011_matrix(
