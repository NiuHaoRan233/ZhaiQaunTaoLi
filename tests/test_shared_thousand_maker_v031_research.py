...
from zhaiquant.shared_thousand_maker_v031_research import (
    MODEL_ID,
    PARENT_MODEL_ID,
)
...
class SharedThousandMakerV031ResearchTests(unittest.TestCase):
    def _engine(self, database: Path, *, code: str = "132026.SH"):
        ...
        strategy_id = maker_comparison_strategy_id(
            config, code, SHARED_THOUSAND_POLICY_V031_CANDIDATE,
        )
        engine = MakerPaperEngine(
            config, store, bond_code=code,
            priority_policy=SHARED_THOUSAND_POLICY_V031_CANDIDATE,
    ...
    def test_v031_is_a_non_mutating_merge_of_v03_and_repaired_v011(self) -> None:
        self.assertEqual(MODEL_ID, "maker_shared_1000_v0_31_candidate")
        self.assertEqual(PARENT_MODEL_ID, SHARED_THOUSAND_POLICY_V03_CANDIDATE.model_id)
        self.assertEqual(
            SHARED_THOUSAND_POLICY_V031_CANDIDATE.model_version,
            "0.31-candidate",
        )
        self.assertTrue(
            SHARED_THOUSAND_POLICY_V031_CANDIDATE
        ...
        self.assertEqual(
            SHARED_THOUSAND_POLICY_V031_CANDIDATE
                .guarded_live_exit_ordinary_lot_kinds,
            ("low_bid_reversion",),
        )
        for unchanged in (
            SHARED_THOUSAND_POLICY_V01_CANDIDATE,
    ...
    def test_ordinary_parent_exit_gap_receives_passive_fallback_next_frame(self) -> None:
        moment = datetime(2026, 9, 2, 11, 14, 41, tzinfo=SHANGHAI)
        with tempfile.TemporaryDirectory() as temporary:
            engine, store = self._engine(Path(temporary) / "ordinary.sqlite3")
            try:
                engine._start_date(moment.date().isoformat())
                fill_tick = _tick(1, moment)
    ...
    def test_session_resilient_lot_keeps_its_native_exit_identity(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            engine, store = self._engine(
                Path(temporary) / "resilient.sqlite3", code="132024.SH",
            )
            try:
                engine._start_date(moment.date().isoformat())
                account, lot = self._open_lot(
                    engine,
                    _tick(1, moment, code="132024.SH"),
    ...
    def test_v03_target_frame_still_creates_session_resilient_order(self) -> None:
        moment = datetime(2026, 9, 2, 10, 44, 3, tzinfo=SHANGHAI)
        with tempfile.TemporaryDirectory() as temporary:
            engine, store = self._engine(
                Path(temporary) / "target.sqlite3", code="132024.SH",
            )
            try:
                engine._start_date(moment.date().isoformat())
                tick = _tick(
                    1, moment, code="132024.SH", bid=136.501,
                    ask=137.199,
                    bids=(
                        (136.501, 2_000.0),
                        (136.500, 1_000.0),
                ...
                self.assertEqual(
                    account.buy_order.kind, "session_resilient_value_entry",
                )
                self.assertAlmostEqual(account.buy_order.limit_price, 136.502)
            finally:
                store.close()
